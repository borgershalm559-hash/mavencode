...
function TabFromQuery({ onTab }: { onTab: (t: "login" | "register") => void }) {
  const searchParams = useSearchParams();
  useEffect(() => {
    if (searchParams.get("tab") === "register") onTab("register");
  }, [searchParams, onTab]);
  return null;
}
        ...
        <div className="relative bg-black border-2 border-[#10B981]/60 p-7 flex-col justify-between min-h-[560px] hidden lg:flex">
          <div>
            <div className="flex items-center gap-3">
              <img src="/logo.svg" alt="MavenCode" width={64} height={50} className="flex-shrink-0" />
            ...
            <div className="mt-10">
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  {isSignin ? (
                    <>Войди.<br />Продолжи.<br /><span style={{ color: ACCENT }}>Реши.</span></>
                  ) : (
                    <>Создай.<br />Сломай.<br /><span style={{ color: ACCENT }}>Построй.</span></>
                  )}
                </motion.h1>
              </AnimatePresence>
              ...
              <div className="mt-5 font-mono text-[12px] leading-relaxed text-white/55 max-w-[280px] space-y-0.5">
                {isSignin ? (
                  <><div>// продолжи курс,</div><div>// прими вызов,</div><div>// защити стрик.</div></>
                ) : (
                  <><div>// 14 дней бесплатно,</div><div>// без карты,</div><div>// отмена в один клик.</div></>
                )}
              </div>
            </div>
          </div>
